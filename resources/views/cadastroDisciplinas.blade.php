...
<head>
    ...
    <title>IFSync | Cadastrar Disciplinas</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        h2 { color: var(--texto-principal); font-weight: 700; margin-bottom: 25px; }
        ...
        .form-container {
            ...
            width: 100%;
            max-width: 650px;
            box-shadow: 0 4px 20px rgba(10, 49, 91, 0.1);
        }
        ...
        .form-check-label { color: var(--texto-principal); font-weight: 500; }
        ...
        .btn-success:hover { background-color: #26763b; }
        ...
        .btn-secondary:hover { background-color: #0f1e2fff; color: var(--bg-principal); }
...
<body>
    ...
    {{-- Conteúdo principal --}}
    <div class="main">
        <h2><i class="bi bi-journal-plus"></i> Cadastrar Disciplinas</h2>

        {{-- Alerts de sucesso/erro --}}
        @if (session('success'))
            <div class="alert alert-success w-100" style="max-width:650px;">
                {{ session('success') }}
            </div>
        @endif

        @if ($errors->any())
            <div class="alert alert-danger w-100" style="max-width:650px;">
                <ul class="mb-0">
        ...
        <div class="form-container">
            <form method="POST" action="{{ route('salvarDisciplinas') }}">
                @csrf

                {{-- Período letivo --}}
                <div class="mb-3">
                    <label for="periodo" class="form-label">Selecione o Período Letivo:</label>
                    <select name="periodo" id="periodo" required class="form-select" onchange="mostrarDisciplinas(this.value)">
                        <option value="">-- Ano/Semestre --</option>
                        @foreach($periodos as $periodo)
                            <option value="{{ $periodo->nome }}" @selected(old('periodo') === $periodo->nome)>
                                {{ $periodo->nome }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Disciplinas --}}
                <div class="mt-4" id="bloco-disciplinas" style="display: none;">
                    <h4 class="h5 mb-3">Selecione suas disciplinas:</h4>
                        ...
                        @foreach($disciplinas as $disciplina)
                            <div class="form-check mb-2">
                                <input
                                    ...
                                    @checked(collect(old('disciplinas', []))->contains($disciplina->id))
                                >
                                <label class="form-check-label" for="disciplina{{ $disciplina->id }}">
                ...
                <div class="d-flex gap-2 mt-4">
                    ...
                    <button type="submit" class="btn btn-success">Salvar Disciplinas</button>
                </div>
            </form>
        </div>
    ...
    <script>
function mostrarDisciplinas(valor) {
    const bloco = document.getElementById('bloco-disciplinas');

    // Mostra apenas se o valor for exatamente "2025.2"
    if (valor === '2025.2') {
